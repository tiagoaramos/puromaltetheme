...
class sellfie_Contact_Info extends WP_Widget {


// constructor

    function sellfie_contact_info() { 

		$widget_ops = array('classname' => 'sellfie_contact_info_widget', 'description' => esc_html__( 'Display your contact info', 'sellfie') ); 

        parent::__construct(false, $name = esc_html__('MT - Contact Info Widget', 'sellfie'), $widget_ops);

		$this->alt_option_name = 'sellfie_contact_info';
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'address' )); ?>"><?php esc_html_e( 'Enter your address', 'sellfie' ); ?></label>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'phone' )); ?>"><?php esc_html_e( 'Enter your phone number', 'sellfie' ); ?></label>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'email' )); ?>"><?php esc_html_e( 'Enter your email address', 'sellfie' ); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['sellfie_contact_info']) )

			delete_option('sellfie_contact_info');		  
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('sellfie_contact_info', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'sellfie_contact_info', 'widget' );
		...
		$title = ( ! empty( $instance['title'] ) ) ? esc_attr( $instance['title'] ) : esc_html__( 'Contact info', 'sellfie' );
		...
		if( ($address) ) {
			...
			echo '<span>' . esc_html__('Address: ', 'sellfie') . '</span>' . wp_kses_post( $address );
		...
		if( ($phone) ) {
			...
			echo '<span>' . esc_html__('Phone: ', 'sellfie') . '</span>' . wp_kses_post( $phone ); 
		...
		if( ($email) ) {
			...
			echo '<span>' . esc_html__('Email: ', 'sellfie') . '</span>' . '<a href="mailto:' . sanitize_email( $email ) . '">' . sanitize_email( $email ) . '</a>';
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'sellfie_contact_info', $cache, 'widget' );  
