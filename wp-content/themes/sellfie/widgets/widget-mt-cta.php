...
class sellfie_action extends WP_Widget {



// constructor

    function sellfie_action() {

		$widget_ops = array('classname' => 'sellfie_action_widget', 'description' => esc_html__( 'Create a call-to-action for a home page widget area.', 'sellfie') );

        parent::__construct(false, $name = esc_html__('MT - Call-to-Action', 'sellfie'), $widget_ops);  

		$this->alt_option_name = 'sellfie_action_widget';
	...
	<label for="<?php echo sanitize_text_field( $this->get_field_id('action_text')); ?>"><?php esc_html_e('Enter your call to action.', 'sellfie'); ?></label>
	...
	<label for="<?php echo sanitize_text_field( $this->get_field_id('action_btn_link')); ?>"><?php esc_html_e('Link for the button', 'sellfie'); ?></label>
	...
	<label for="<?php echo sanitize_text_field( $this->get_field_id('action_btn_text')); ?>"><?php esc_html_e('Title for the button', 'sellfie'); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['sellfie_action']) )

			delete_option('sellfie_action');		  
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('sellfie_action', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'sellfie_action', 'widget' );
...
?>
            	...
            	<div class="col-1-1">
                	...
                	<div class="home-call-to-action">
                        ...
                        <?php if ($action_btn_link !='') : ?>
							...
							<a href="<?php echo esc_url( $action_btn_link ); ?>" class="button"><?php echo esc_html( $action_btn_text ); ?></a>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'sellfie_action', $cache, 'widget' ); 
