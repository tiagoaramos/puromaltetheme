...
if (!isset($_SESSION['commentinput'])||$_SESSION['commentinput']=='') {
...
if (function_exists('add_theme_support')) {
	...
	set_post_thumbnail_size( 660, 230 , true );
...
if ( current_user_can('administrator') ) {
	...
	function wow_excerpt($args='', $postid=''){
		global $post, $WOWTheme;
			...
			if ( isset($i['maxchar']) ) {
				...
				if ( $p->post_excerpt ) {
					...
					$content = $p->post_content;
					$content = apply_filters('the_content', $content);
					$maxchar=($WOWTheme->get( 'layout','cuttxton' ))?$WOWTheme->get( 'layout','cuttxt' ):0;
	...
	add_filter('wp_list_categories', 'wow_menu');
	add_filter('wp_list_pages', 'wow_menu');
	add_filter('wp_nav_menu_items', 'wow_menu');
	function block_main_menu() {
	global $WOWTheme;
		?>
		 <div class="menu-primary-container">
			<ul class="menus menu-primary">
                <li <?php if(is_home() || is_front_page()) { ?>class="current_page_item"<?php } ?>><a href="<?php echo home_url(); ?>/"><?php echo $WOWTheme->_(  'homelink' );?></a></li>
	...
	function wow_mobile_menu($menu_name) {
...
function addGMap($atts, $content = null) {
        ...
        extract(shortcode_atts(array( 'address' => '', 'type' => 'hybrid', 'zoom' => '16'  ), $atts));
        $rand=uniqid();
		$id='map_canvas'.mktime().rand(0,1000);
		$output='<div class="googlemap" id="'.$rand.'"></div><script>gglMap("'.$rand.'", "'.$address.'", "'.$type.'", "'.$zoom.'");</script>';
		return $output;
		

}
add_shortcode('map', 'addGMap');
...
function addSoundCloud($atts, $content = null) {
        extract(shortcode_atts(array( "id" => '', 'url' =>'', 'params'=>'' ), $atts));
		if ( $id>0 ) $url='http%3A%2F%2Fapi.soundcloud.com%2Ftracks%2F'.$id;
		if ( $params!='' ) $url.='&amp;'.$params;
			else $url.='&amp;color=ea5854&auto_play=false&show_artwork=false';
		return '<p style="text-align:center;display:block;overflow:hidden;clear:both;"><iframe width="100%" height="166" scrolling="no" frameborder="no" src="https://w.soundcloud.com/player/?url='.$url.'"></iframe></p>';
}
add_shortcode('soundcloud', 'addSoundCloud');

function add_wowpanel() {
   if ( ! current_user_can('edit_posts') && ! current_user_can('edit_pages') )
     return;
   if ( get_user_option('rich_editing') == 'true') {
     add_filter('mce_external_plugins', 'add_wowpanel_tinymce_plugin');
     add_filter('mce_buttons_3', 'register_wowpanel');
   }
}

add_action('init', 'add_wowpanel');
function register_wowpanel($buttons) {
   array_push($buttons, "youtube","vimeo","|","btns","cols","tooltips","highlights", "gmap");
   return $buttons;
}

function add_wowpanel_tinymce_plugin($plugin_array) {
   $plugin_array['wowpanel'] = get_template_directory_uri() .'/js/editor_plugin.js';
   return $plugin_array;
}
...
function wowtheme_topic_count_text($count) {
	global $WOWTheme;
	return sprintf( _n($WOWTheme->_( 'altposts' ), $WOWTheme->_( 'altpostss' ), $count), number_format_i18n( $count ) );
}
function wowtheme_tag_cloud() {
	$args['topic_count_text_callback']='wowtheme_topic_count_text';
	return $args;
}
add_filter('widget_tag_cloud_args', 'wowtheme_tag_cloud');
...
global $WOWTheme;
        switch ( $translated_text ) {
            ...
            case 'View all posts filed under %s' :
                ...
                $translated_text = $WOWTheme->_( 'altcats' );
                break;
