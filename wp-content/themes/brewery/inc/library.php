	...
	class WOWTheme {
		...
		function WOWTheme() {
			...
			if ( !is_admin() ) {
				...
				add_action('init', 'wow_register_scripts');
			}
		...
		function headtext() {
			...
			<?php } ?>
				...
				<script type="text/javascript" src="<?php echo get_template_directory_uri()?>/js/wow.js?ver=<?php echo rand(1,2000);?>"></script>
		...
		function block_social () {
			...
			<ul class='inner'>
				<?php
					...
					if (!isset($img)||$img=='')$img=$this->get( 'general', 'logoimage' );
					foreach ($services as $service) {
						if ($service['show']) {
							$code=preg_replace('/wow_social_url/', $href, $service['code']);
							$code=preg_replace('/wow_social_title/', $this->pagetitle, $code);
							$code=preg_replace('/wow_social_desc/', $this->pagetitle, $code);
							$code=preg_replace('/wow_social_img_url/', $img, $code);
							echo "<li>".$code."</li>";
						}
					}
				?>
 ...
 animation: {'marginLeft':'0px',opacity:'show'},				
	...
	?>
		...
		function getparams($params,$reset) {
			...
			foreach ($sections as $section) {
				...
				$rparam='/'.addcslashes(str_replace('?', '\?', str_replace(' ', '\s',trim($param))),'/').'/';
		...
		function seo() {
			...
			if ( !$this->get( 'seo','wow_seo' ) ) return false;
			$descr=$keywords='';
	...
	function wowthemes_die($msg) {
		switch ((int)$msg) {
			...
			break;
			case 3: die('<center>Settings for wowtheme hasn\'t been reading</center>');
			break;
...
?>
		...
		<div class="comment-meta commentmetadata">
			<?php
				printf( $WOWTheme->_( 'commenttime' ), get_comment_date(),  get_comment_time()) ?>
		</div>
		<?php printf(__('<cite class="author-name">%s</cite>&nbsp;&nbsp;<span class="says">'.$WOWTheme->_( 'says' ).':</span>'), get_comment_author_link()) ?>
		</div>
<?php if ($comment->comment_approved == '0') : ?>
		<em class="comment-awaiting-moderation"><?php $WOWTheme->_( 'commentmoderation' ) ?></em>
		<br />
<?php endif; ?>

		<?php comment_text() ?>

		<div class="reply"><?php edit_comment_link( $WOWTheme->_( 'edit' ),'&nbsp;&nbsp;','&nbsp;|' ) ?>
		<?php comment_reply_link(array_merge( $args, array('reply_text'=> $WOWTheme->_( 'reply' ),'add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))) ?>
		</div>
		<?php if ( 'div' != $args['style'] ) : ?>
		</div>
		<?php endif; ?>
