...
class YITH_YWRAQ_Order_Request {
	...
	public function create_order( $raq ) {
		...
		add_filter( 'woocommerce_is_purchasable', array( $this, 'is_purchasable' ) );
		...
		remove_filter( 'woocommerce_is_purchasable', array( $this, 'is_purchasable' ) );
	...
	public function ajax_accept_order() {
		if ( ! isset( $_POST['order_id'] ) ) {
			...
			if ( $accepted_page_id == $checkout_page_id ) {
				...
				$result = array(
					'result'  => 1,
					'rqa_url' => apply_filters( 'ywraq_accepted_redirect_url', function_exists( 'wc_get_checkout_url' ) ? wc_get_checkout_url() : WC()->cart->get_checkout_url(), $order->id) ,
				);
		...
		if ( $status == 'accepted' && ywraq_verify_token( $_GET['raq_nonce'], 'accept-request-quote', $order_id, $user_email ) ) {
			...
			if ( in_array( $current_status, array( 'ywraq-pending', 'pending', 'ywraq-accepted') ) ) {
				...
				if ( YITH_Request_Quote()->enabled_checkout() ) {
					...
					$redirect = function_exists( 'wc_get_checkout_url' ) ? wc_get_checkout_url() : WC()->cart->get_checkout_url();
